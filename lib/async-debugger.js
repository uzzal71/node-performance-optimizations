...
class AsyncDebugger {
  ...
  startTracking(label, userCode) {
    ...
    executionContext.runInAsyncScope(() => {
      // Initiate a new execution context whose child async operations we will track.
  ...
  dump() {
    ...
    for (const entry of this.numAsyncOps.entries()) {
      // Translate to a hash of labels.
      const [executionContextId, numOps] = entry;
      const label = this.labelMap.get(executionContextId);
      numAsyncOps[label] = numOps;
      asyncOps[label] = allOps.filter(
        (asyncOp) => this.labelMap.get(asyncOp.executionContextId) === label
      );
    }
  ...
  traverse(indentLevel, asyncOp) {
    const lines = [
      `${asyncOp.asyncId} - ${asyncOp.type} - ${asyncOp.status}`,
    ].concat(
      asyncOp.stack.slice(1).map((stackLine) => "  " + stackLine) // Indent the stack.
    );
  ...
  addAsyncOperation(asyncId, type, triggerAsyncId, resource) {
    ...
    //
    this.numAsyncOps.set(
      executionContextId,
      (this.numAsyncOps.get(executionContextId) || 0) + 1
    );

    fs.writeSync(
      1,
      `- Added: ${type} (Id = ${asyncId} - Parent = ${triggerAsyncId}) - Context = ${executionContextId}\n`
    );
  ...
  removeAsyncOperation(asyncId, reason) {
    ...
    if (executionContextId !== undefined) {
      ...
      if (numAsyncOps !== undefined) {
        ...
        fs.writeSync(
          1,
          `   Removed: Id = ${asyncId} - Reason = ${reason} - Context = ${executionContextId}\n`
        );
      }
    }
